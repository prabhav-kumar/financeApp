/**
 * api.ts — Centralized API client for the FinanceIQ backend.
 *
 * All fetch calls go through this module so auth headers,
 * base URL, and error handling are consistent everywhere.
 */
...
export function getToken(): string | null {
  ...
  return localStorage.getItem("financeiq_token");
}
...
export const investment = {
  ...
  add: (data: Partial<Investment>) =>
    request<Investment>("/investment/", { method: "POST", body: JSON.stringify(data) }),
...
export const loan = {
  ...
  add: (data: Partial<Loan>) =>
    request<Loan>("/loan/", { method: "POST", body: JSON.stringify(data) }),
