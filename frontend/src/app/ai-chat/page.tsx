...
function renderContent(content: string) {
  return content.split("\n").map((line, i) => {
    ...
    if (line.trim().startsWith("## "))
      return <p key={i} style={{ fontWeight: 700, fontSize: 13, marginTop: 14, marginBottom: 4, color: "var(--indigo)", textTransform: "uppercase", letterSpacing: "0.04em" }}>{line.replace(/^##\s*/, "")}</p>;
    if (line.trim().startsWith("Problem:") || line.trim().startsWith("Impact:") || line.trim().startsWith("Action:")) {
      const [label, ...rest] = line.split(":");
      return <div key={i} style={{ display: "flex", gap: 6, margin: "3px 0 3px 8px" }}>
        <span style={{ fontWeight: 600, fontSize: 12, color: "var(--text-muted)", minWidth: 52 }}>{label}:</span>
        <span style={{ fontSize: 13 }}>{parts(rest.join(":"))}</span></div>;
    }
    if (line.trim().startsWith("• ") || line.trim().startsWith("- "))
      return <div key={i} style={{ display: "flex", gap: 8, paddingLeft: 8, margin: "3px 0" }}>
        <span style={{ color: "var(--indigo)", flexShrink: 0 }}>•</span>
        <span style={{ fontSize: 13 }}>{parts(line.trim().replace(/^[•\-]\s*/, ""))}</span></div>;
...
export default function AiChatPage() {
  ...
  // Load sessions on mount — always from API so titles persist across navigation
  ...
  const toggleVoice = useCallback(() => {
    ...
    r.onresult = (e: SpeechRecognitionEvent) => {
      ...
      transcriptRef.current = t; setInput(t);
    };
    r.onend = () => {
      setListening(false);
      const t = transcriptRef.current.trim(); transcriptRef.current = "";
  ...
  return (
    <div style={{ display: "flex", height: "calc(100vh - 100px)", overflow: "hidden" }}>
      ...
      {/* ── Sidebar ── */}
      <div style={{ width: sidebarOpen ? 240 : 0, minWidth: sidebarOpen ? 240 : 0, transition: "all 220ms ease", overflow: "hidden", display: "flex", flexDirection: "column", background: "var(--bg-card)", borderRight: "1px solid var(--border)", borderRadius: "var(--r-lg) 0 0 var(--r-lg)" }}>
        ...
        <div style={{ padding: "4px 10px 6px" }}>
          <p style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em" }}>Recent Chats</p>
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "0 6px 12px" }}>
          {loadingSessions ? Array.from({ length: 4 }).map((_, i) => (
            ...
            <div key={s.id} onClick={() => openSession(s.id)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 8px", borderRadius: 8, cursor: "pointer", margin: "2px 0", background: activeSession?.id === s.id ? "rgba(99,102,241,0.1)" : "transparent", border: activeSession?.id === s.id ? "1px solid rgba(99,102,241,0.2)" : "1px solid transparent", transition: "all 150ms ease" }}
              ...
              <button onClick={(e) => deleteSession(s.id, e)} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, opacity: 0.4, flexShrink: 0 }}
        ...
        <div style={{ padding: "10px 12px", borderTop: "1px solid var(--border)" }}>
          <p style={{ fontSize: 11, color: "var(--text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user?.full_name}</p>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
          ...
          <div>
            <p style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)" }}>{activeSession?.title || "DhanSathi AI"}</p>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: 14 }}>
          {!activeSession ? (
            <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, marginTop: 80 }}>
              ...
              <p style={{ fontSize: 13, color: "var(--text-muted)", textAlign: "center", maxWidth: 320 }}>Start a new chat or select a previous conversation.</p>
              <button onClick={newChat} className="btn btn-primary" style={{ gap: 8 }}><Plus size={14} /> Start New Chat</button>
            </div>
          ) : (
            <>
              {activeSession.messages.map((msg) => (
                <div key={msg.id} className="fade-in" style={{ display: "flex", gap: 10, flexDirection: msg.role === "user" ? "row-reverse" : "row" }}>
                  <div style={{ width: 30, height: 30, borderRadius: "var(--r-sm)", flexShrink: 0, background: msg.role === "user" ? "var(--grad-teal)" : "var(--grad-primary)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {msg.role === "user" ? <User size={14} color="white" /> : <Sparkles size={14} color="white" />}
                  </div>
                  <div style={{ maxWidth: "82%" }}>
                    <div style={{ padding: msg.role === "user" ? "10px 14px" : "14px 16px", borderRadius: "var(--r-lg)", background: msg.role === "user" ? "rgba(99,102,241,0.12)" : "var(--bg-card)", border: `1px solid ${msg.role === "user" ? "rgba(99,102,241,0.25)" : "var(--border)"}`, fontSize: 13, lineHeight: 1.7, color: "var(--text-primary)" }}>
                      {msg.role === "user" ? <p style={{ whiteSpace: "pre-wrap" }}>{msg.content}</p> : renderContent(msg.content)}
                    </div>
              ...
              {loading && (
                <div className="fade-in" style={{ display: "flex", gap: 10 }}>
                  <div style={{ width: 30, height: 30, borderRadius: "var(--r-sm)", background: "var(--grad-primary)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Sparkles size={14} color="white" /></div>
              ...
              {activeSession.messages.length <= 1 && !loading && (
                <div className="fade-in" style={{ marginTop: 8 }}>
                  <p style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 8, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em" }}>Suggested questions</p>
        ...
        {activeSession && (
          <div style={{ padding: "12px 16px", borderTop: "1px solid var(--border)", flexShrink: 0 }}>
            ...
            <div className="card" style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px" }}>
              ...
              {voiceSupported && (
                <button onClick={toggleVoice} disabled={loading} title={listening ? "Stop" : "Speak"}
                  style={{ width: 34, height: 34, borderRadius: "var(--r-sm)", flexShrink: 0, border: listening ? "none" : "1px solid var(--border)", background: listening ? "rgba(239,68,68,0.15)" : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 200ms var(--ease)" }}>
                  {listening ? <MicOff size={15} color="var(--red)" /> : <Mic size={15} color="var(--text-muted)" />}
                </button>
              )}
              <button onClick={handleSend} disabled={loading || !input.trim() || listening}
                style={{ width: 34, height: 34, borderRadius: "var(--r-sm)", flexShrink: 0, border: input.trim() ? "none" : "1px solid var(--border)", background: input.trim() ? "var(--grad-primary)" : "transparent", cursor: input.trim() ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", opacity: input.trim() ? 1 : 0.35, transition: "all 200ms var(--ease)" }}>
